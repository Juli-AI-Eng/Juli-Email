...
describe('Tool Approval Flows', () => {
  ...
  beforeAll(async () => {
    ...
    const hasCredentials = !!E2E_CONFIG.nylas.apiKey && !!E2E_CONFIG.nylas.grantId;
    if (!hasCredentials) {
      throw new Error('Missing required credentials. Set NYLAS_ACCESS_TOKEN and NYLAS_GRANT_ID.');
    }
    ...
    client = createTestClient({
      port: server.port,
      credentials: {
        nylasAccessToken: E2E_CONFIG.nylas.apiKey,
